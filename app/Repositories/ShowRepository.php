...
class ShowRepository extends Repository
{
    public function getAllShows(): array
    {
        try {
            $sql = "
                SELECT 
                    s.show_id, s.show_name, s.start_date, s.price, 
                    s.location_id, s.available_spots,
                    l.venue_name,
                    a.artist_id, a.name AS artist_name, a.genre AS artist_genre
                FROM `SHOW` s
    ...
    public function getShowById(int $id)
    {
        try {
            $sql = "
                SELECT show_id, show_name, start_date, price, location_id, available_spots 
                FROM `SHOW` 
                WHERE show_id = :id 
                LIMIT 1
            ";
    ...
    public function createShow(array $data): int
    {
        try {
            $sql = "
                INSERT INTO `SHOW` (show_name, start_date, price, location_id, available_spots) 
    ...
    public function getShowsForArtist(int $artistId): array
    {
        try {
            $sql = "
                SELECT 
                    s.show_id, s.show_name, s.start_date, s.price, 
                    s.available_spots, l.venue_name, 
            ...
            error_log("getShowsForArtist error: " . $e->getMessage());
            return [];
        }
    }
}